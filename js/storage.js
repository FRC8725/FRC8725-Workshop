// js/storage.js — cabinet detail page

import { $, el, escapeHtml, icon, formatLocation, isLowStock, isOutOfStock } from "./utils.js";
...
import { renderStructure } from "./storage-renderer.js";
import { getViewMode, setViewMode, renderViewSwitcher } from "./item-view.js";
...
async function boot() {
  ...
  if (!storageId) return renderError("未指定櫃子", "網址缺少 id 參數。");
  ...
  host.innerHTML = `
    <div class="storage-title-wrap">
      <div class="storage-meta">
        <a class="btn btn-ghost btn-sm" href="index.html">← 返回平面圖</a>
      </div>
...
async function handleDelete(item) {
  ...
  try {
    ...
    console.error(err);
    notify.danger("刪除失敗：" + (err.message || "未知錯誤"));
  }
}
...
function renderError(title, msg) {
  ...
  main.innerHTML = `
    <div class="error-page">
      ...
      <p>${escapeHtml(msg)}</p>
      <a class="btn btn-primary" href="index.html">返回平面圖</a>
    </div>
  `;
}
